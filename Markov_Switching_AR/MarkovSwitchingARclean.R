...
condLogLik <- function(theta, series) {
  ...
  # probability to stay in state 2 c1, c2, constant term for AR process in resp.
  # state 1 and 2 phi sigma, standard deviation of Gaussian disturbances returns :
  ...
  logf <- sum(log(f[5:n]))
  # cat(logf, '\n') flush.console() output <- cbind(eta, f, dzeta)
  return(-logf)
}
...
test <- condLogLik(theta <- startVal, series <- data[2:725, 5])
...
###################################### Forecasting ###################
#point forecast for period 2011-2012
# (726-1183) 1# static approach create forecasts and calculate errors matrix with
# estimate, error, estimated variance
static <- cbind(rep(0, 458), rep(0, 458))
# names(static) <- c('estimate', 'error')

estimate <- constrOptim(c(0.8, 0.8, -0.0002646, -0.0002646, 4.6171e-06, 4.6171e-06, 
  0.072568, 0.072568, 0.91988, 0.91988), ui = constraintMat, ci = constraintVec, 
  condLogLik, series = logRetFirstPeriod, grad = NULL)
...
for (i in 726:1183) {
  ...
  # [1] p11,[2] p22,[3] c1,[4] c2,[5] phi11,[6] phi12,[7] phi21, [8] phi22,[9]
  # phi31,[10] phi32,[11] phi41,[12] phi42,[13] sigma1,[14] sigma2
  ...
  # point estimate
  mean1 <- curPar[3] + curPar[5] * data$logRet[i - 1] + curPar[7] * data$logRet[i - 
    1] + curPar[9] * data$logRet[i - 1] + curPar[11] * data$logRet[i - 1]
  mean2 <- curPar[4] + curPar[6] * data$logRet[i - 1] + curPar[8] * data$logRet[i - 
    1] + curPar[10] * data$logRet[i - 1] + curPar[12] * data$logRet[i - 1]
...
# Compute predicted 95 percent confidence intervals calculate 2,5 and 97,5
# quantiles
...
## Checking univformity of density forecasts Density transformation
u <- c(rep(0, 458))
for (i in 1:length(u)) {
...
################################################### 3# reestimation, rolling window reestimate the parameters for the longer time
################################################### series
MSAR4last12 <- optim(f = condLogLik, p <- prevPar, series = data[(725 - 240):(725), 
  5])
recursivePar <- MAAR4last12$par  # estimated parameter values for last 12 months of in-sample period
prevPar <- recursivePar

for (i in 726:1183) {
  reestMSAR412month <- optim(f = condLogLik, p <- prevPar, series = data[(i - 241):(i - 
    1), 5])
  recursivePar <- cbind(recursivePar, reestMSAR4$par)
...
for (i in 726:1183) {
  # Forecasting values for i calculate dzeta[i], sigma[i]
  info <- condLogLikDzeta(theta <- recursivePar[, (i - 725)], series <- data[(i - 
    241):(i - 1), 5])  #calculate dzeta and sigma
  curPar <- recursivePar[, (i - 725)]  # estimated parameters 
  # [1] p11,[2] p22,[3] c1,[4] c2,[5] phi11,[6] phi12,[7] phi21, [8] phi22,[9]
  # phi31,[10] phi32,[11] phi41,[12] phi42,[13] sigma1,[14] sigma2
  ...
  sigma2Forecast[i - 725] <- p1 * (curPar[5] + (curPar[7] * info[[4]][length(info)]) + 
    curPar[9] * sigma2Cur) + p2 * (curPar[6] + (curPar[8] * info[[5]][length(info)]) + 
    curPar[10] * sigma2Cur)
}
...
# predicted 95 percent confidence intervals calculate 2,5 and 97,5 quantiles
confInterval <- cbind(rep(0, 458), rep(0, 458))
for (i in 1:458) {
